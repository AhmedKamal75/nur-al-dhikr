/**
 * icons.js — the app's icon system.
 *
 * A small set of hand-drawn (not copied) line icons as inline SVG strings,
 * keyed by name. Inline (not a sprite) is deliberate: the patch engine
 * keeps matched nodes alive across renders, so inline SVGs never flicker,
 * and `currentColor` inheritance keeps every icon theme-aware for free.
 *
 * Conventions every entry in PATHS follows:
 *   - 24×24 viewBox; stroke-based line art; stroke-width (1.7), round
 *     caps/joins and `fill="none"` come from the single `icon()` wrapper —
 *     paths only carry geometry (and, where needed, explicit overrides).
 *   - Solid shapes opt OUT of stroke explicitly (`stroke="none"` +
 *     `fill="currentColor"`): play / stop / pause, the tasbih bead, the
 *     target bullseye and the prayer-rug dot.
 *   - One drawing per glyph. A second NAME for the same shape is an ALIASES
 *     entry, never a copy-pasted path (the integrity gate enforces this).
 *   - No dead markup: nothing ships at opacity 0 "just in case" (the gate
 *     catches this class of bug too — sunset carried one for years).
 *
 * The standing gate (tests/helpers/icon-audit.mjs + tests/icons.test.js;
 * repo-root scripts/icon-audit.mjs is a CLI shim) fails
 * when any referenced name — static `icon('x')` calls, ternaries,
 * `icon:` map values, exported icon maps, or "icon" fields in data/*.json —
 * is missing here, when two PATHS entries carry identical path data, or
 * when an alias points at a non-existent glyph. A typo must never again
 * ship as the blank-square fallback.
 */
...
export const PATHS = {
  ...
  eyeOff:
    '<path d="M4 4l16 16"/><path d="M9.9 5.2A9.8 9.8 0 0 1 12 6c6 0 9.5 6 9.5 6a17 17 0 0 1-2.2 2.9M6.3 7.5A16.6 16.6 0 0 0 2.5 12s3.5 6 9.5 6a9.6 9.6 0 0 0 3.4-.6"/><path d="M9.9 9.9a2.6 2.6 0 0 0 3.7 3.7"/>',
  heart:
  ...
  // (v4.5) The classic bulleted list — the study-reader glyph. This name
  // was ALREADY referenced by the v4.4 action sheet's "Reader View" row
  // (passed through a variable, so the icon gate never saw the literal)
  // and rendered as a silent blank square; defining it repairs that too.
  ...
  sunset:
    '<path d="m6 10 1.5 1.5M18 10l-1.5 1.5"/><circle cx="12" cy="14" r="4"/><path d="M3 20.5h18"/><path d="M9 3.5h6"/>',
  ...
  calculator:
    '<rect x="5" y="3" width="14" height="18" rx="2"/><path d="M8.5 7h7"/><path d="M8.5 11.5v.01M12 11.5v.01M15.5 11.5v.01M8.5 15v.01M12 15v.01M15.5 15v.01M8.5 18v.01M12 18v.01M15.5 18v.01"/>',
  ...
  repeat:
    '<path d="M17 2.5 21 6l-4 3.5"/><path d="M21 6H7a4 4 0 0 0-4 4v1"/><path d="M7 21.5 3 18l4-3.5"/><path d="M3 18h14a4 4 0 0 0 4-4v-1"/>',
  folder:
    '<path d="M3.5 6.5A1.5 1.5 0 0 1 5 5h4l2 2.5h8a1.5 1.5 0 0 1 1.5 1.5v9A1.5 1.5 0 0 1 19 19.5H5a1.5 1.5 0 0 1-1.5-1.5Z"/>',
  coins:
    '<ellipse cx="9" cy="7" rx="5.5" ry="2.5"/><path d="M3.5 7v4c0 1.4 2.5 2.5 5.5 2.5s5.5-1.1 5.5-2.5V7"/><path d="M3.5 11v4c0 1.4 2.5 2.5 5.5 2.5 1 0 2-.1 2.8-.4"/><path d="M14.5 15.5h6M17.5 12.5v6"/>',
  quran:
  ...
  // v4.4 — location profiles (work/travel), the journal, mastery badges,
  // and the certificate print affordance.
  briefcase:
    '<rect x="3.5" y="8" width="17" height="11" rx="2"/><path d="M9 8V6.5A1.5 1.5 0 0 1 10.5 5h3A1.5 1.5 0 0 1 15 6.5V8"/><path d="M3.5 12.5h17"/>',
  plane:
    '<path d="M10.5 13.5 3 11l1.5-1.5 6 .5 4.5-5.5c.8-.9 2.3-.4 2.3.8L15 12l5 1.5-1.5 1.5-6-1-3.5 4"/>',
  feather:
    '<path d="M20 4c-5 0-11 3-12.5 10.5L5 21"/><path d="M20 4c0 5-3 10-9 11h4"/><path d="M14 10l-5 5"/>',
  award:
    '<circle cx="12" cy="9" r="5.5"/><path d="m8.5 13.5-1.5 7 5-2.5 5 2.5-1.5-7"/><path d="m10 9 1.5 1.5L14.5 7.5"/>',
  printer:
    '<path d="M7 8V4h10v4"/><rect x="4" y="8" width="16" height="8" rx="1.5"/><path d="M7 13h10v7H7Z"/>',
  // (v4.5) TRUE fullscreen mushaf: enter/exit glyphs, drawn as the classic
  // four-corner bracket pair used by video players and book readers.
  expand:
    '<path d="M9 4H5.5A1.5 1.5 0 0 0 4 5.5V9"/><path d="M15 4h3.5A1.5 1.5 0 0 1 20 5.5V9"/><path d="M15 20h3.5a1.5 1.5 0 0 0 1.5-1.5V15"/><path d="M9 20H5.5A1.5 1.5 0 0 1 4 18.5V15"/>',
  compress:
    '<path d="M4 9h3.5A1.5 1.5 0 0 0 9 7.5V4"/><path d="M20 9h-3.5A1.5 1.5 0 0 1 15 7.5V4"/><path d="M20 15h-3.5a1.5 1.5 0 0 0-1.5 1.5V20"/><path d="M4 15h3.5A1.5 1.5 0 0 1 9 16.5V20"/>',
  // (v4.5) The mushaf page medallion and the juz margin glyph.
  rosette:
    '<path d="M12 2.5 14 5l3.4-.6.6 3.4 2.5 2.2-2.5 2.2-.6 3.4-3.4-.6L12 20.5 10 18.4l-3.4.6-.6-3.4L3.5 13l2.5-2.2.6-3.4 3.4.6Z"/>',
  ...
  sprout:
    '<path d="M12 21V10"/><path d="M12 10C12 6 9.5 4 5.5 4c0 4 2.5 6 6.5 6Z"/><path d="M12 13c0-3.5 2.5-5.5 6.5-5.5 0 3.5-2.5 5.5-6.5 5.5Z"/><path d="M7 21h10"/>',
  tree: '<path d="M12 21v-6"/><path d="M12 3.5 7.5 9h3L6 14.5h5V21"/><path d="M12 3.5 16.5 9h-3l4.5 5.5h-5"/><path d="M8 21h8"/>',
};

/**
 * Deliberate second names for the same glyph. Data files and view code
 * historically use both words for the same drawing ("food" in the duas
 * corpus, "rain" in moods) — aliases keep those references readable
 * without ever duplicating path data. Every value must exist in PATHS
 * (the integrity gate enforces it).
 */
export const ALIASES = {
  food: 'utensils',
  rain: 'cloud-rain',
  // v4.1 — the load-failure affordances (ui/emptyState.js): reuse the
  // existing glyphs rather than inventing near-duplicates.
  cloudOff: 'cloud-rain', // "network trouble" medallion
  refresh: 'repeat', // circular-arrows retry button
};

const FALLBACK = '<rect x="4" y="4" width="16" height="16" rx="2"/>';

/** Return an inline <svg> string for the given icon name (falls back to a blank square for unknown names — the audit makes sure this never happens in real usage). */
export function icon(name, { size = 20, className = '' } = {}) {
  const body = PATHS[name] || PATHS[ALIASES[name]] || FALLBACK;
