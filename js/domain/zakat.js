...
/** Length of a lunar (hijri) year in days — the FALLBACK hawl length.
 *  (v4.3) the true anniversary is now computed on the tabular Hijri
 *  calendar (same date, next Hijri year), which tracks the real ~354.37-day
 *  lunar year; a flat 354 drifted ~11 days per 30-year cycle. */
export const HAWL_DAYS = 354;

/**
 * The hawl anniversary of a zakat assessment: the SAME Hijri date one Hijri
 * year after the assessment date (tabular calendar), falling back to a flat
 * 354-day year when the calendar conversion cannot produce a valid date.
 * Zakat becomes due again each time a full hawl passes on wealth still at
 * or above nisab.
 */
export function hawlDueFor(assessmentTs) {
  const assessment = new Date(assessmentTs);
  if (Number.isFinite(assessment.getTime())) {
    try {
      const h = toHijri(assessment);
      if (h && Number.isFinite(h.year) && Number.isFinite(h.month) && Number.isFinite(h.day)) {
        const anniversary = toGregorian(h.year + 1, h.month, h.day);
        const ts = anniversary instanceof Date ? anniversary.getTime() : NaN;
        if (Number.isFinite(ts)) return ts;
      }
    } catch {
      /* fall through to the flat-year fallback */
    }
  }
