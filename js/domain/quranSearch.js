 ...
 *
 * (v4.2) MEMOIZED on the last (query, limit): the linear corpus scan is
 * fast (~2ms desktop) but it ran on EVERY re-render of the search view —
 * including unrelated dispatches (audio state flips, bookmark saves) while
 * the same query was standing. Consecutive renders of one query are the
 * overwhelming common case; a one-entry cache covers them for free.
 */
