 ...
 */
export const TAJWEED_FAMILIES = Object.freeze([
  {
    id: 'silent',
    color: '#9E9E9E',
  ...
  },
  {
    id: 'nasal',
    color: '#4CAF50',
    name: { en: 'Ghunnah family (nasalization)', ar: 'عائلة الغنة' },
    desc: {
      en: 'Ghunnah, ikhfa, iqlab and the ghunnah idghams — all nasal, all green.',
      ar: 'الغنة والإخفاء والإقلاب والإدغام بغنة — كلها أنفية وكلها خضراء.',
    },
  },
  {
    id: 'qalqalah',
    color: '#00BCD4',
    name: { en: 'Qalqalah (echo)', ar: 'القلقلة' },
    desc: {
      en: 'The bouncing echo on ق ط ب ج د at sukun (cyan).',
      ar: 'اهتزاز حروف قطب جد الساكنة (سماوي).',
    },
  },
  {
    id: 'heavy',
    color: '#2196F3',
    name: { en: 'Tafkhim (heavy)', ar: 'التفخيم' },
    desc: {
      en: 'The heavy lam of the divine name and a heavy ra\u2019 (blue).',
      ar: 'لام لفظ الجلالة والراء المفخمة (أزرق).',
    },
  },
  {
    id: 'madd',
...
export const TAJWEED_RULES = Object.freeze([
  ...
  },
  {
    ...
    name: { en: 'Tafkhim (heavy)', ar: 'التفخيم' },
    desc: {
      en: 'The heavy lam of لفظ الجلالة (الله) and a ra\u2019 carrying fatha/damma.',
      ar: 'لام لفظ الجلالة، والراء المفخمة المفتوحة أو المضمومة.',
    },
  },
  {
    id: 'madd_2',
  ...
  },
  {
    ...
    desc: {
      en: 'At a pause, a final fathah-tanween is read as an alif — 2 counts.',
      ar: 'عند الوقف يُبدل تنوين الفتح ألفًا، بمقدار حركتين.',
    },
  },
  {
    id: 'madd_badal',
...
export function classifyWordTajweed(
  ...
  for (let i = 0; i < units.length; i += 1) {
    ...
    if (u.base === RA && (u.diacritics.has(FATHA) || u.diacritics.has(DAMMA))) {
      spans.push({ start: u.start, end: u.end, rule: 'tafkhim' });
    }

    // (v4.5.2) Madd 'iwad — at a PAUSE on the ayah's final word, the fathah
    // tanween is recited as an alif of 2 counts (compensating madd). The
    // tanween mark itself may sit on the last unit (رَبًّا without its
    // written alif) or on the unit before a trailing bare tanween-alif
 ...
 *
 * (v4.2) MEMOIZED: classification is pure and deterministic over immutable
 * text, yet it ran on EVERY render of every visible ayah — the classic
 * reader's full-surah rebuild re-classified all of Al-Baqarah (~19ms) on
 * each dispatch (play/pause, bookmark notes, hifz taps). Keyed by the ayah
 * text itself: the corpus is immutable, so a hit is always correct, and the
 * cache is naturally bounded by the 6,236-ayah corpus (~small objects).
 */
...
/** Safe read of the prefs slice (absent/hostile → empty = all standard). */
export function tajweedPrefsOf(state) {
  const p = state?.settings?.tajweedPrefs;
  return p && typeof p === 'object' && !Array.isArray(p) ? p : {};
}

/** A rule is active unless explicitly switched off (all-on default). */
export function ruleEnabled(prefs, ruleId) {
  return prefs?.rules?.[ruleId] !== false;
}

/** The effective color of a rule: family override > rule default. */
export function effectiveRuleColor(prefs, rule) {
  if (!rule) return null;
  const familyOverride = rule.family ? prefs?.colors?.[rule.family] : null;
  return typeof familyOverride === 'string' && /^#[0-9a-fA-F]{6}$/.test(familyOverride)
    ? familyOverride
    : rule.color;
}
