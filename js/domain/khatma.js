...
export function planStatus({ pagesRead, plan, today = new Date() }) {
  ...
  // (v4.3) `remaining` pages after TODAY'S reading are finished at
  // ceil(remaining / pace) more days — the old `- 1` treated today's
  // not-yet-done pages as if they were already read, so near-deadline
  // "on track" verdicts could flip on the off-by-one.
  let projectedFinishISO = null;
  if (pace > 0 && remaining > 0) {
    projectedFinishISO = plusDays(today, Math.ceil(remaining / pace));
