...
import { PALETTES, SHAPES, THEME_MODES, QURAN_RECITERS, TRANSLATION_EDITIONS } from '../config.js';
...
export function renderSettings(state) {
  ...
  const paletteSwatches = PALETTES.map(
    (p) => `
    <button type="button" class="swatch ${s.palette === p.id ? 'swatch--active' : ''}" style="--sw-color:${p.primary}" data-action="set-setting" data-key="palette" data-value="${p.id}" aria-label="${escapeHTML(pickLocale(p.name, lang))}" title="${escapeHTML(pickLocale(p.name, lang))}"></button>`
  ).join('');
  ...
  const modeButtons = THEME_MODES.map(
    (m) => `
    <button type="button" class="segmented__btn ${s.themeMode === m ? 'segmented__btn--active' : ''}" data-action="set-setting" data-key="themeMode" data-value="${m}">${t('settings.themeMode.' + m, lang)}</button>`
  ).join('');

  const langButtons = availableLanguages()
    .map(
      (l) => `
    <button type="button" class="segmented__btn ${s.language === l ? 'segmented__btn--active' : ''}" data-action="set-setting" data-key="language" data-value="${l}">${l === 'en' ? 'English' : '\u0627\u0644\u0639\u0631\u0628\u064A\u0629'}</button>`
    )
    .join('');
  ...
  const translationRows = TRANSLATION_EDITIONS.map(
    (ed) => `
    <button type="button" class="reciter-row ${s.quranTranslation === ed.id ? 'reciter-row--active' : ''}" data-action="set-setting" data-key="quranTranslation" data-value="${ed.id}" dir="auto">
  ...
  <section class="view view--settings">
    ...
    <section class="panel">
      <div class="panel__header"><h2>${t('settings.soundHaptics', lang)}</h2></div>
      ...
      <p class="panel__subtext">${t('settings.soundHint', lang)}</p>
    </section>
    ...
    <section class="panel">
      ...
      <p class="field-label">${t('settings.quranTranslation', lang)}</p>
      <div class="reciter-list">${translationRows}</div>
      <p class="panel__subtext">${t('settings.quranTranslationHint', lang)}</p>
