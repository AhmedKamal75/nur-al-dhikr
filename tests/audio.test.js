...
import {
  pad3, surahUrl, customMoshafId, validateCustomServer, searchReciters
...
test('surahUrl builds canonical per-surah URLs (no double slashes)', () => {
  assert.equal(surahUrl('https://server8.mp3quran.net/afs/', 1), 'https://server8.mp3quran.net/afs/001.mp3');
  assert.equal(surahUrl('https://download.quranicaudio.com/quran/husary_muallim', 114), 'https://download.quranicaudio.com/quran/husary_muallim/114.mp3');
});
...
test('searchReciters matches Arabic names diacritic-insensitively (after catalog load)', async () => {
  ...
  const customs = [{ id: 'custom-x', nameEn: 'Alaa Aql', nameAr: 'علاء عقل', server: 'https://x/' }];
