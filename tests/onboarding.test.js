...
test('buildOnboardingSteps: all six steps start undone for a fresh user', () => {
  const steps = buildOnboardingSteps(baseState());
  assert.deepEqual(
    steps.map((s) => s.id),
    ['location', 'notifications', 'prayer', 'goals', 'install', 'firstReading']
  );
  assert.deepEqual(
    steps.map((s) => s.done),
  ...
  assert.deepEqual(CONFIRM_STEPS, ['prayer', 'goals']);
});
...
test('buildOnboardingSteps: each completion signal flips exactly its own step', () => {
  ...
  const setUp = buildOnboardingSteps(
    baseState({ onboarding: { dismissed: false, stepsSeen: { prayer: true, goals: true } } })
  );
  assert.deepEqual(
    setUp.map((s) => s.done),
...
test('onboardingComplete: only when every step is done', () => {
  const all = buildOnboardingSteps(
    baseState({
      ...
      onboarding: { dismissed: false, stepsSeen: { prayer: true, goals: true } },
